/***************************************************************************
  tag: Peter Soetens  Mon May 10 19:10:38 CEST 2004  ReportingComponent.cxx
    ...
    modified by          : Dennis Leroy Wigand (dwigand@cor-lab.de)
...
// ORO_LIST_COMPONENT_TYPE(cogimon::ReportingComponent)
...
namespace cogimon
{
     ...
     */
    bool memberDecomposition( base::DataSourceBase::shared_ptr dsb, RTT::PropertyBag& targetbag, DataSource<bool>::shared_ptr& resized)
    {
        assert(dsb);
        ...
        unique_ptr< Property<RTT::PropertyBag> > recurse_bag( new Property<RTT::PropertyBag>("recurse_bag","") );
        ...
        for(vector<string>::iterator it = parts.begin(); it != parts.end(); ++it ) {
            ...
            // In case of a string, do not split it into chars!
            if (!newpb->getType().compare("string")) {
  ...
  ReportingComponent::ReportingComponent( std::string name /*= "Reporting" */ )
        : TaskContext( name ),
          ...
          report_data("ReportData","A RTT::PropertyBag which defines which ports or components to report."),
    ...
    bool ReportingComponent::configureHook()
    {
        ...
        RTT::PropertyBag bag = report_data.value();
        ...
        bool ok = true;
        RTT::PropertyBag::const_iterator it = bag.getProperties().begin();
    ...
    bool ReportingComponent::screenImpl( const std::string& comp, std::ostream& output)
    {
        ...
        RTT::PropertyBag* bag = c->properties();
        if (bag) {
